...
void drawhand(int length, int width, double theta){
	...
	Point vhand[4];
	vhand[0]=add(screen->r.min, add(catoffs, Pt(CATWID/2+iround(length*s), CATHGT/2-iround(length*c))));
	vhand[1]=add(screen->r.min, add(catoffs, Pt(CATWID/2-iround(ws+wc), CATHGT/2+iround(wc-ws))));
	vhand[2]=add(screen->r.min, add(catoffs, Pt(CATWID/2-iround(ws-wc), CATHGT/2+iround(wc+ws))));
	vhand[3] = vhand[0];
